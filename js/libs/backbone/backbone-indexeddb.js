(function () { /*global _: false, Backbone: false */
  ...
  Backbone.Driver = function (schema, ready, nolog) {
  ...
  // Driver Prototype
  _.extend(Backbone.Driver.prototype,{
    //Driver.prototype = {
    ...
    execute: function (storeName, method, object, options) {
      if (!this.nolog) debugLog("execute : " + method +  " on " + storeName + " for " + object.id+ 'searchID: '+options.searchId);
      switch (method) {
        case "create":
        ...
        case "delete":
          if (object.id || object.cid) {
            this.deleteQuery(storeName, object, options);
          } else {
    ...
    read: function (storeName, object, options) {
      ...
      var getRequest = null;
      if(options.searchId){
        getRequest = store.get(options.searchId);
      }
      else if (json.id) {
    ...
    deleteQuery: function (storeName, object, options) {
      ...
      var deleteRequest = store.deleteQuery(json.id);
          ...
          if (!cursor) {
            ...
            if (options.limit && processed >= options.limit) {
              ...
              if (options.addIndividually) {
                ...
                var deleteRequest = store.deleteQuery(cursor.value.id);
                deleteRequest.onsuccess = function (event) {
  ...
  function ExecutionQueue(schema,next,nolog) {
    this.driver     = new Backbone.Driver(schema, this.ready.bind(this), true);
    this.started    = false;
    this.stack      = [];
  ...
  var Databases = {};
  Backbone.indexDBSync = function (method, object, options){
    ...
    options.success = function(resp) {
      ...
      object.trigger('Backbone.indexDBSync', object, resp, options);
    };
