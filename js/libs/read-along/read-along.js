...
var ReadAlong = {
  ...
  getCurrentWord: function () {
    ...
    var len;
    var is_current_word;
    var is_first_word = false;
    var is_last_word = false;
    var word = null;
    for (i = 0, len = this.words.length; i < len; i += 1) {
      is_current_word = (
        ...
        || 
        (
          this.audio_element.currentTime < this.words[i].begin && (i!=0 || i!= (this.words.length-1))
          )
        
        );
      
      // current word is the first one. but we don't want to select the word 
      if(this.audio_element.currentTime < this.words[i].begin && i==0){
        is_first_word = true;
        word = this.words[i].begin;
        break;
      }
      
      // current word is the first one. but we don't want to select the word 
      if(this.audio_element.currentTime > this.words[i].end && i==(this.words.length-1)){
        is_last_word = true;
        word = true;
        break;
      }
  ...
  selectCurrentWord: function() {
    ...
    var seconds_until_this_word_ends;

    // time > last word.end
    if(typeof current_word === 'boolean'){
      this.removeWordSelection();
      return;
    }
    
    // first word.start < time < last word.end 
    if(typeof current_word !== 'number'){
      if (typeof current_word !== 'undefined'){
	    //image 
	    if(current_word.element.style.position=='absolute') {
	  ...
	  //word
		  ...
		  this.removeWordSelection();
		  current_word.element.classList.add('speakingword');
		  if (this.autofocus_current_word) {
			current_word.element.focus();
		  }
	  }
    ...
    if (is_playing) {
      ...
      if(typeof current_word === 'number')
        seconds_until_this_word_ends = current_word;
      else
        seconds_until_this_word_ends = current_word.end - this.audio_element.currentTime; // Note: 'word' not 'world'! ;-)
        ...
        function () {
          if (!that.audio_element.paused) { // we always want to have a word selected while paused
            if(typeof current_word !== 'number')
			{
			  ...
			  current_word.element.classList.remove('speakingword');
			}
          }
        },
      ...
      var next_word ;
      if(typeof current_word === 'number')
        next_word = this.words[0];
      else
        next_word = this.words[current_word.index + 1];
  ...
  removeWordSelection: function() {
    ...
    spoken_word_els = this.text_element.querySelectorAll('span[data-begin].speakingword');
    Array.prototype.forEach.call(spoken_word_els, function (spoken_word_el) {
      spoken_word_el.classList.remove('speakingword');
    });
  },
  ...
  addEventListeners: function () {
    ...
    that.audio_element.addEventListener('play', function (e) {
      that.selectCurrentWord();
	  if( that.text_element.style.position=='absolute') 
		that.text_element.classList.add('speaking');
	  else
		that.text_element.classList.add('speakingword');
    }, false);
    ...
    that.audio_element.addEventListener('pause', function (e) {
	  ...
	  that.text_element.classList.remove('speakingword');
    }, false);
